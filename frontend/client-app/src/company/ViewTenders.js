...
import ViewQuotation from "../company/ViewQuotation";
import ReactDom from "react-dom/client";
function ViewTenders(props){
 ...
 useEffect(()=>{
    ...
    axios.get("http://localhost:9190/tender/showbycompanyid/"+props.data.cuserid).then((res)=>{
        setTList(res.data);
    }).catch((err)=>{
    ...
    const handleViewQuotButton=(tenderid,tenderbyid)=>{
        var obj={tid:tenderid,
            tenderbyid:tenderbyid
        }
        const root = ReactDom.createRoot(document.getElementById("root"));
        root.render(<ViewQuotation data={obj}></ViewQuotation>)
    ...
    <table border={10} style={{backgroundColor:"black",color:"white"}}>
        ...
        </tr>
        {
            tlist.map((titem)=>(
               ...
               <td>
                <button type="submit" onClick={()=>handleViewQuotButton(titem.tenderid,titem.tenderbyid)}>View Quotation</button>
               </td>
                </tr>
            ))
        }
